        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (data[i].pictures.length > 0)
                    content += `<a href="/ownerManager/homeDetail.html?id=${data[i].id}"><img src="${data[i].pictures[0].src}"></a>`
                else content += `<a href="/ownerManager/homeDetail.html?id=${data[i].id}"><img src="https://upload.wikimedia.org/wikipedia/commons/b/bd/Queen_Boudica_by_John_Opie.jpg"></a>`

                content += `</div>
                                    <div class="item-entry overflow">
                                        <h5><a href="/ownerManager/homeDetail.html?id=${data[i].id}">${data[i].name}</a></h5>
                                        <div class="dot-hr"></div>`
                if (data[i].types.length > 0)
                    content += `
                                        <span class="pull-left"><b>Home type: </b>${data[i].types[0].name}</span>
                                        `
